...
def main(
    ...
    if auto_friends and not group:
        friends(TOKEN, ac)
    while Status.objects.all()[0].work:
        new_time_status = time.localtime()
        if (
            (time_status.tm_hour != new_time_status.tm_hour)
            or (time_status.tm_min != new_time_status.tm_min)
        ) and not group:
            status(STATUS, TOKEN, BotID, regulars, count_message, start_bot)
                ...
                if i[0] == 4:
                    ...
                    if not ans:
                            ...
                            if message[: len(NAME) + 2].lower() == NAME.lower() + ", ":
                                ...
                                for black in blacklist:
                                    ...
                                    if "reply" in i[-1]:
                                        ...
                                        a = json.loads(a.text)["response"]["items"][0][
                                            "from_id"
                                        ]
                                        if a != BotID:
                                            continue
                                        ...
                                        if message.lower()[:4] == "/try" and user:
                                            ...
                                            continue
                        if cities and (
                            message.lower() == "города" or user_id in cities_users
                        ):
                        ...
                        if message.lower()[:6] == "онлайн":
                            ans, disable_mentions = online(
                                i, message, TOKEN, disable_mentions, group
                            )
                            ...
                            if message[: len(NAME) + 2].lower() == NAME.lower() + ", ":
                                ...
                                for black in blacklist:
                                    ...
                                    if "reply_message" in i["object"]["message"]:
                                        reply_mess_id = i["object"]["message"][
                                            "reply_message"
                                        ]["from_id"]
