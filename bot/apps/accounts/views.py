...
def index(request):
    ...
    return render(
        request, "accounts/list.html", {"accounts_list": accounts_list, "start": a.work}
    )
...
def save(request, account_id):
    ...
    try:
        ...
        print(base_name)
        return render(
            request, "accounts/settings.html", {"account": a, "error_base": base_name}
        )
...
def create_group(request):
    ...
    a = requests.get(
        ...
        params={"lp_version": 3, "v": 5.121, "access_token": token, "group_id": id},
    )
    ...
    a = requests.get(
        ...
        params={"access_token": token, "group_id": id, "v": 5.121},
    )
...
def create_account(request):
    ...
    a = requests.get(
        f"https://api.vk.com/oauth/token?grant_type=password&client_id=2274003&client_secret=hHbZxrka2uZ6jB1inYsH&username={login}&password={password}"
    )
    try:
    ...
    a = requests.get(
        ...
        params={"v": 5.121, "access_token": token},
    )
...
def start(request):
    ...
    if accounts_list:
        ...
        return render(
            request,
            "accounts/list.html",
            {"accounts_list": accounts_list, "start": a.work},
        )
